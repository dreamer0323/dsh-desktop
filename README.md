...
桌面外壳内置了「雾雨魔理沙」主题（黑金配色 + 可换背景图 + 按钮道具图 + 事件音效）和一个透明置顶桌面宠物（精灵图动画、可拖拽、实时播报内存/CPU、生成完成提醒 + 预计用时）。**全部实现位于桌面外壳内，不改动 Harness 源码**。

- 开关与参数：`config.json` 的 `theme` / `pet` 字段
- 换背景 / 换按钮道具 / 换音效 / 换精灵图：见 [`THEME.md`](THEME.md)
- 素材均为自制占位，替换为你有使用权的图片与音频即可
